...
void Phase_1()
{
        ...
        printf("\nPhase 1: Bidder1 %s has TCP Port %d  and IP address %s", bidd1.szBidderName, ntohs(addrBidd.sin_port), inet_ntoa(addrBidd.sin_addr));
        fflush(stdout);
        ...
        strcat(szLogIn, "1 ");
        strcat(szLogIn, bidd1.szBidderName);
        strcat(szLogIn, " ");
        strcat(szLogIn, bidd1.szBidderPass);    
        strcat(szLogIn, " ");
        strcat(szLogIn, bidd1.szBidderAcc);
        
        printf("\nPhase 1: Login request. User: %s password: %s Bank account: %s", bidd1.szBidderName, bidd1.szBidderPass, bidd1.szBidderAcc);
...
void ReadBidder(char *pszBidderFile)
{
    ...
    while( i < 1 )
    {
        fscanf(fp,"%d %s %s %s", &bidd1.iType, bidd1.szBidderName, bidd1.szBidderPass, bidd1.szBidderAcc);                
        i++;
    }        
...
void Phase_3()
{
    int iSockId, iPort = 4933, iBuffRx;    
    char buffRx[1024], buffTx[1024], szIP[20];;
    struct sockaddr_in addrBidd1, addrAucServ;
    socklen_t ServLen = sizeof(addrAucServ);
    
    memset(&addrBidd1, 0, sizeof(addrBidd1));
    addrBidd1.sin_family = AF_INET;
    addrBidd1.sin_port = htons(iPort);
    addrBidd1.sin_addr.s_addr = inet_addr("127.0.0.1");
    
    iSockId = socket(AF_INET, SOCK_DGRAM, 0);
    
    if(bind(iSockId, (struct sockaddr *) &addrBidd1, sizeof(addrBidd1)) < 0)
    {
        perror("Bind Client:");
        return;
    }      
    
    printf("\nPhase 3: Bidder1 %s has UDP port %d and IP address %s", bidd1.szBidderName, iPort, inet_ntoa(addrBidd1.sin_addr));
    fflush(stdout);
    ...
    ReadBiddingInfo("bidding1.txt", buffTx);
...
void ReadBiddingInfo(char *pszFile, char *buffTx)
{
    ...
    strcat(buffTx, bidd1.szBidderName);
